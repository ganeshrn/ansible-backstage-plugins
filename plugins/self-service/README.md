...
The self-service plugin is the frontend plugin for Red Hat Ansible Automation Platform.
First page (catalog) is available at `/self-service/catalog`.
...
Your plugin has been added to the example app in this repository, meaning you'll be able to access it by running `yarn start` in the root directory, and then navigating to [/self-service](http://localhost:3000/self-service).
...
### Additional configuration for sidebar
