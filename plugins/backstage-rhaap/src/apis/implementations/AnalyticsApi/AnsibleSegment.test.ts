...
import { AnsibleSegmentAnalytics } from './AnsibleSegment';
...
describe('AnsibleSegmentAnalytics', () => {
  const context = {
    extension: 'App',
    pluginId: 'ansible',
    routeRef: 'unknown',
  ...
  describe('fromConfig', () => {
    it('returns implementation', () => {
      const api = AnsibleSegmentAnalytics.fromConfig(basicValidConfig);
  ...
  describe('integration', () => {
    ...
    it('track identify with maskedIP', async () => {
      const api = AnsibleSegmentAnalytics.fromConfig(maskedIPConfig);
      await api.captureEvent({
        action: 'identify',
        subject: 'jdoe',
        context,
